...
class AuteurModel{
    ...
    function findAllAuteurs(){
        $results=[];
        $requete = "SELECT au.nom, au.prenom, au.id, count(oe.id) as nbrOeuvre
        FROM OEUVRE oe
    ...
    function createAndPersistAuteur($donnees){
        $requete=" INSERT INTO AUTEUR (id,nom,prenom) VALUES (NULL,'${donnees['nom']}','${donnees['prenom']}');";
...
//        ');DELETE FROM AUTEUR WHERE id BETWEEN 20 AND 80;
        ...
        $requete=" INSERT INTO AUTEUR (id,nom,prenom) VALUES (NULL,:nom,:prenom);";
        $prep=$this->db->prepare($requete);
...
//        // https://www.pierre-giraud.com/php-mysql-apprendre-coder-cours/requete-preparee/
    ...
    function removeByIdAuteur($id) {
        $requete="DELETE FROM AUTEUR WHERE id=:id LIMIT 1;";
        $prep=$this->db->prepare($requete);
    ...
    function findOneByIdAuteur($id){
        $requete="SELECT id,nom,prenom FROM AUTEUR WHERE id=:id;";
        $prep=$this->db->prepare($requete);
        $prep->bindParam(':id', $id, \PDO::PARAM_INT);
        $prep->execute();
        $result=$prep->fetch();
    ...
    function updateAndPersistAuteur($id, $donnees){
        $requete="UPDATE AUTEUR set nom=:nom, prenom=:prenom WHERE id=:id;";
        $prep=$this->db->prepare($requete);
        $prep->bindParam(':id', $id, \PDO::PARAM_INT);
        $prep->bindParam(':nom', $donnees['nom'], \PDO::PARAM_STR_CHAR);
        $prep->bindParam(':prenom', $donnees['prenom'], \PDO::PARAM_STR_CHAR);
